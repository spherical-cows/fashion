...
PresentedBy.prototype.draw = function () {
  ...
  atom.context.drawImage(stars, 0, 0, atom.width, atom.height);
...
LoseScreen.prototype.draw = function() {
  ...
  var draw_leaderboard = function(header, col, offsetpx) {
	  ...
	  for (var i = 0; i < self.score.scoreboard().length; i++) {
		  var text = self.score.scoreboard()[i][col];
		  
		  atom.context.fillStyle = self.score.scoreboard()[i]["default"] ? 'red' : cycle_colours[Math.floor(Math.random() * cycle_colours.length)];
