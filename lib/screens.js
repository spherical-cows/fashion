...
SplashScreen.prototype.update = function () {
	if (Object.keys(atom.input._pressed).length > 0) {
		...
		var challenges = [];
		for (var x = 0; x < 10; x++) {
		  var sentence = corpus.sentence();
		  challenges.push(new Challenge(sentence.split('')));
		}
...
function newAlbert(path, frameCount, opts) {
  ...
  return new Animation(albertSprites, opts.speed || 5, 0, frameCount, opts.reversible || false)
}
...
var GameplayScreen = function(challenges) {
  ...
  this.startTimer = function () {
	  this.timer = new Timer(this.currentChallenge.length() * 500, this.gameOver);
  }
  this.startTimer();
...
GameplayScreen.prototype.theCrowdTakesMorePhotos = function() {
  for (var i=0; i < this.correctWithoutError + this.correctWithoutError * Math.round(Math.random() * 3); i++) {
    setTimeout(function() {
...
>>>>>>> Al : Error sound effect when pressing incorrect key
